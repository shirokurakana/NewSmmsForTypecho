 ...
 * 
 * @package NewSmms
 * @author ranlychan, shirokurakana
 * @version 1.2.1
 * @link https://thdog.moe
 * @dependence 1.2-*
 * @date 2023-12-1
 */
...
class NewSmmsForTypecho_Plugin extends Widget_Upload implements Typecho_Plugin_Interface
{
    ...
    {
        $smmsHost = new Typecho_Widget_Helper_Form_Element_Radio(
            'smmsHost',
            array(Smms::API_URL_1 => 'sm.ms', Smms::API_URL_2 => 'smms.app'),
            Smms::API_URL_2,                          // 默认选中项
            '服务域名',                                 // radio组标题
            _t('sm.ms服务国外，smms.app服务国内')
        );      // 说明文字
        $form->addInput($smmsHost);

        $smmsToken = new Typecho_Widget_Helper_Form_Element_Password(
            'smmsToken',
            NULL,
            NULL,
            _t('API Token：'),
            _t('去<a href="https://smms.app/" target="_blank">https://smms.app/</a>注册登录账号后，点击右上角User->Dashboard，新页面点击左侧API Token，得到Secret Token 禁用插件后需要重写填写，但原来上传的图片不受影响')
        );
        ...
        $smmsUploadExts = new Typecho_Widget_Helper_Form_Element_Text(
            'smmsUploadExts',
            NULL,
     ...
     * @return mixed
     */
    public static function pUploadHandle(array $file)
    {
        if (empty($file['name'])) {
            return false;
        }
     ...
     * @return mixed
     */
    public static function pModifyHandle(array $content, array $file)
    {
        if (empty($file['name'])) {
            return false;
        }
     ...
     * @return string
     */
    public static function pAttachmentHandle(array $content): string
    {
     ...
     * @return string
     */
    public static function pAttachmentDataHandle(array $content): string
    {
     ...
     * @return boolean
     */
    protected static function makeUploadDir(string $path): bool
    {
